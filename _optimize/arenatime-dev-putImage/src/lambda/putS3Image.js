...
"use strict";const datePath=require("../util/datePath"),aws=require("aws-sdk"),s3=new aws.S3,moment=require("moment"),fileType=require("file-type"),uuid=require("uuid/v4");let getFile=function(fileMime,buf){let fileExt=fileMime.ext,filePath=datePath(),fileName=uuid()+"."+fileExt;return{Bucket:"priconne-arenatime",Body:buf,Key:"images/"+(filePath+fileName),ContentEncoding:"base64"}};module.exports.handler=(event,context)=>{let once=event.body.match(/data:image\/\w+;base64,.+/)[0].replace(/data:image\/\w+;base64,/,""),twice=once.replace(/\n------WebKitFormBoundary.+/,""),buffer=Buffer.from(twice,"base64"),fileMime=fileType(buffer),params=getFile(fileMime,buffer);s3.putObject(params).promise().then(()=>{let response={statusCode:200,body:JSON.stringify({message:params.Key,runtime:context}),headers:{"Access-Control-Allow-Origin":"https://stdev17.github.io","Access-Control-Allow-Credentials":!0}};context.succeed(response)}).catch(()=>{let response={statusCode:400,body:JSON.stringify({message:"Upload Failed",runtime:context}),headers:{"Access-Control-Allow-Origin":"https://stdev17.github.io","Access-Control-Allow-Credentials":!0}};context.fail(response)})};

},{"../util/datePath":8,"aws-sdk":undefined,"file-type":1,"moment":3,"uuid/v4":6}],8:[function(require,module,exports){
"use strict";var moment=require("moment"),datePath=function(){let year=moment().year(),month=moment().month()+1,day=moment().date();return year+"/"+month+"/"+day+"/"};module.exports=datePath;
